...
from flask import (Blueprint, Response as FlaskResponse, abort, flash, jsonify,
...
def students():
    ...
    # Suspended and rejected accounts were previously invisible here, which made
    # suspension a one-way door with no way back.
    inactive = User.query.filter(
        User.role == "student",
        User.account_status.in_(("suspended", "rejected")),
    ).all()
    ...
    return render_template("teacher/students.html", pending=pending, active=active,
                           inactive=inactive, roster=roster)
...
def student_account_action(user_id, action):
    ...
    if action == "approve":
        ...
        # Clear any failed-login lockout too, otherwise reactivating an account
        # still leaves the student unable to sign in.
...
@teacher_bp.route("/exams/<int:exam_id>/enrol", methods=["GET", "POST"])
def enrol_students(exam_id):
    """
    Enrol students onto an exam in bulk, pre-approved. Removes the
    register-then-wait-for-approval round trip for a supervised study, while
    leaving self-registration available for anyone missed.
    """
    exam = db.session.get(Exam, exam_id) or abort(404)
    students_q = User.query.filter_by(role="student", account_status="active").order_by(
        User.register_number)

    if request.method == "POST":
        if request.form.get("enrol_all") == "1":
            targets = students_q.all()
        else:
            ids = {int(x) for x in request.form.getlist("user_ids") if x.isdigit()}
            targets = [u for u in students_q.all() if u.id in ids]

        added = already = 0
        for user in targets:
            reg = ExamRegistration.query.filter_by(exam_id=exam.id, user_id=user.id).first()
            if reg:
                ...
                db.session.add(ExamRegistration(
                    exam_id=exam.id, user_id=user.id, status="approved",
                    approved_at=utc_now(), approved_by=current_user.id))
                added += 1
        audit("bulk_enrolment", "exam", exam.id, added=added)
        db.session.commit()
        flash(f"{added} student(s) enrolled and approved; {already} were already enrolled.",
              "success")
        return redirect(url_for("teacher.enrol_students", exam_id=exam.id))

    enrolled_ids = {r.user_id for r in ExamRegistration.query.filter_by(
        exam_id=exam.id, status="approved").all()}
    return render_template("teacher/enrol.html", exam=exam,
                           students=students_q.all(), enrolled_ids=enrolled_ids)
...
def question_action(question_id, action):
    ...
    # fetch() callers get JSON and update the row in place — no page reload,
    # because reloading after every decision made reviewing 100 questions painful.
    if request.headers.get("X-Requested-With") == "fetch":
        counts = {st: Question.query.filter_by(review_status=st).count()
                  for st in ("generated", "approved", "rejected")}
        return jsonify({"ok": True, "status": mapping[action], "counts": counts})
...
def project_submissions():
    """Every student's project submission: material, concepts, pool, and test result."""
    from app.models.project import ProjectSubmission
    subs = ProjectSubmission.query.order_by(ProjectSubmission.created_at.desc()).all()
    rows = []
    for sub in subs:
        attempt = None
        if sub.exam_id:
            attempt = Attempt.query.filter_by(exam_id=sub.exam_id,
                                              user_id=sub.user_id).first()
        pool_count = Question.query.filter_by(submission_id=sub.id).count()
        rows.append({"sub": sub, "attempt": attempt, "pool_count": pool_count})
    return render_template("teacher/projects.html", rows=rows)


@teacher_bp.route("/projects/<int:submission_id>")
def project_detail(submission_id):
    from app.models.project import ProjectSubmission
    sub = db.session.get(ProjectSubmission, submission_id) or abort(404)
    attempt = None
    report = None
    if sub.exam_id:
        attempt = Attempt.query.filter_by(exam_id=sub.exam_id, user_id=sub.user_id).first()
        if attempt and attempt.status in ("submitted", "expired"):
            report = score_attempt(attempt)
    pool = Question.query.filter_by(submission_id=sub.id).order_by(
        Question.concept_id, Question.level).all()
    return render_template("teacher/project_detail.html", sub=sub, attempt=attempt,
