...
RULES:
- "options" must contain four FULL answer texts, not letters or labels.
  WRONG: ["A", "B", "C", "D"]  or  ["Option A", "Option B", ...]
  RIGHT: ["To uniquely identify each row", "To sort the table alphabetically", ...]
- "answer" is the 0-based index of the correct option.
- All four options must be plausible to someone who has not studied the topic.
  Do not write obviously silly distractors.
- Do not reveal the answer inside the question text.

Return ONLY valid JSON, no prose, following this SHAPE (not this content):
{{
  "question": "Why is a primary key required in a relational table?",
  "options": [
    "To uniquely identify each row",
    "To store the table in alphabetical order",
    "To compress the stored data",
    "To encrypt sensitive columns"
  ],
  "answer": 0,
  "explanation": "A primary key guarantees each tuple can be referenced unambiguously.",
  "options_breakdown": {{
    "0": "Correct — uniqueness is exactly what a primary key enforces.",
    "1": "Incorrect — ordering is handled by indexes and ORDER BY, not keys.",
    "2": "Incorrect — keys do not affect storage size.",
    "3": "Incorrect — encryption is unrelated to key constraints."
  }},
  "level": {level}
}}
...
RULES:
- "options" must contain four FULL answer texts, never letters or labels.
  WRONG: ["A", "B", "C", "D"].  RIGHT: ["To uniquely identify each row", ...]
- "answer" is the 0-based index of the correct option.

Return ONLY valid JSON, no prose, following this SHAPE (not this content):
{{
  "question": "Which statement about primary keys is correct?",
  "options": [
    "A primary key value must be unique in its table",
    "A primary key may repeat across rows",
    "A primary key must always be text",
    "A primary key is optional in every design"
  ],
  "answer": 0,
  "explanation": "Uniqueness is the defining property of a primary key.",
  "options_breakdown": {{
    "0": "Correct — uniqueness is guaranteed by the constraint.",
    "1": "Incorrect — repetition is what the constraint prevents.",
    "2": "Incorrect — keys can be numeric or text.",
    "3": "Incorrect — without one, rows cannot be referenced reliably."
  }},
  "level": {level}
}}
