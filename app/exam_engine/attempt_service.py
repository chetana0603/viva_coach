...
def assign_next_question(attempt, exam, target_level, remedial_for=None,
    ...
    # sequence_number must be unique per attempt and strictly increasing over ALL
    # assigned questions. It cannot be derived from current_sequence, because
    # remedials deliberately do not increment that — doing so gave a remedial and
    # the primary question after it the same number, violating uq_assigned_sequence.
    next_sequence = max((a.sequence_number for a in attempt.assigned), default=0) + 1
...
def submit_answer(attempt, exam, assigned_question_id, selected_option, client_response_time=None):
    ...
    if wants_remedial:
        # Try the same concept a level easier, then the same concept at any level.
        # Pinning to one concept AND one level means a thin bank silently skips
        # remediation altogether — the student never sees the second chance the
        # scoring model assumes exists.
        ...
        if next_aq is None and easier != aq.assigned_level:
            next_aq = assign_next_question(attempt, exam, aq.assigned_level,
                                           remedial_for=aq,
                                           concept_id=aq.question.concept_id)
    ...
    if next_aq is None and attempt.current_sequence <= attempt.total_answered:
        # Distinguish "finished the paper" from "the question bank ran dry", or a
        # short test looks like correct behaviour when it is really a thin bank.
        completed = attempt.current_sequence >= exam.question_count
        finalise_attempt(attempt,
                         reason="manual" if completed else "bank_exhausted")
