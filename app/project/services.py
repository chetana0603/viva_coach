...
def generate_question_pool(submission, per_level=POOL_PER_LEVEL, levels=POOL_LEVELS):
    ...
    own material. Runs straight after analysis — study cards are a subject-viva
    feature, not part of the project flow. Questions are auto-approved because
    they are private to this student's test; every one still passes the same
    validator (placeholder options, distractor quality, grounding) before saving.
    """
    if submission.status not in ("analysed", "cards_ready", "questions_ready"):
